...
import 'package:flutter/material.dart';
import 'Dashboard.dart'; // Import models from Dashboard.dart

class HealthOverview extends StatelessWidget {
  final HealthDashboardResponse data;
  final bool isLoading;

  const HealthOverview({super.key, required this.data, this.isLoading = false});
  ...
  Widget build(BuildContext context) {
    ...
    if (!data.isSuccess || data.data == null) {
      return Text(
        'Error fetching health data: ${data.message}',
        style: const TextStyle(fontSize: 14, color: Colors.red),
      );
    }
      ...
      child: Column(
        ...
        children: [
          RowItem(
            ...
            value: health.projectStatus,
          ),
          const Divider(height: 8, color: Colors.grey),
          RowItem(
            label: 'Time',
            value: health.timeStatus,
          ),
          const Divider(height: 8, color: Colors.grey),
          RowItem(
            label: 'Tasks',
          ...
          RowItem(
            label: 'Progress',
            value: '${health.progressPercent.toStringAsFixed(1)}% complete',
          ),
          const Divider(height: 8, color: Colors.grey),
          RowItem(
            label: 'Cost Performance Index',
            value: health.costStatus == 0 ? 'No budget specified' : '${health.costStatus}',
          ),
          const Divider(height: 8, color: Colors.grey),
          RowItem(
            label: 'Schedule Performance Index',
            value: health.cost.schedulePerformanceIndex.toStringAsFixed(2),
          ),
        ],
      ),
    );
